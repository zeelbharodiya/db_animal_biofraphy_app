...
class Sea_Page extends StatefulWidget {
  const Sea_Page({Key? key}) : super(key: key);
...
class _Sea_PageState extends State<Sea_Page> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Sea Animal",style: TextStyle(fontSize: 20,color: Colors.white,fontWeight: FontWeight.w400),),
      ...
      body: Container(
        child: GridView.builder(
          ...
          itemCount: Global.all4.length,
          itemBuilder: (context, i) {
            return GestureDetector(
              onTap: () {
                setState(() {
                  Navigator.pushNamed(context, 'Detail_Page', arguments: Global.all4[i]);
                });
              },
              child: Card(
                elevation: 5,
